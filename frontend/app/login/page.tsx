...
function LoginForm() {
  ...
  // Whitelisted so a crafted `next` value can never redirect off-platform —
  // only known internal routes are honored. Falls back to prior behavior
  // (`/onboarding` / `/dashboard`) if absent or unrecognized.
  const ALLOWED_NEXT = ['/assessment', '/onboarding', '/dashboard'];
  const nextParam = params.get('next');
  const resolveNext = (fallback: string) =>
    nextParam && ALLOWED_NEXT.includes(nextParam) ? nextParam : fallback;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        setSuccess(true);
        setTimeout(() => router.push(resolveNext('/onboarding')), 600);
      } else {
        await login(email, password);
        setSuccess(true);
        const { githubConnected, jiraConnected } = await getConnectionStatus();
        setTimeout(() => {
          router.push(resolveNext(githubConnected && jiraConnected ? '/dashboard' : '/onboarding'));
        }, 600);
      }
    } catch {
        ...
        <Card className={`p-8 shadow-2xl transition-all duration-300 ${error ? 'animate-shake border-accent-rose/40' : ''}`}>
          <div className="mb-6">
            ...
            <h1 className="font-display text-2xl font-bold mt-1 text-text-primary">
              {isRegister ? 'Set up your workspace' : 'Log in to Patchline X'}
            </h1>
