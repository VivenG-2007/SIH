...
to a Mongo regex scan over this ORGANIZATION's scan_history when ES is
...
Scoped by organizationId (P0#1), not ownerId — any member of the
organization can search findings any member's scan produced, matching the
same "shared team resource, not per-user silo" model the rest of the
risk-engine's write paths already use.
"""
...
async def _mongo_fallback_search(organization_id: str, q: str, severity: Optional[str], repo: Optional[str], size: int) -> list[dict]:
    db = get_db()
    mongo_filter: dict[str, Any] = {"organizationId": organization_id}
    if repo:
...
async def search(
    ...
    es_results = await es_client.search_findings(
        organization_id=user.org_id, query=q, severity=severity, repo=repo, status=status, size=limit
    )
    if es_results is not None:
        return {"results": es_results, "source": "elasticsearch", "total": len(es_results)}

    fallback = await _mongo_fallback_search(user.org_id, q, severity, repo, limit)
