...
sqli-py-concat (semgrep-rules/patchlinex-rules.yml) only fires on a genuine
...
def _parse_results(payload: dict, path_map: dict[str, str], target_dir: Path) -> list[dict]:
    ...
    for result in payload.get("results", []):
        ...
        # pack's path (e.g. "semgrep-rules.patchlinex-rules.sqli-py-fstring")
        # — strip back to the bare id we authored (see patchlinex-rules.yml)
...
def scan_paths(files: list[dict]) -> list[dict]:
    ...
    with tempfile.TemporaryDirectory(prefix="patchlinex-semgrep-") as tmp:
        target_dir = Path(tmp)
