...
async def lifespan(app: FastAPI):
    ...
    # CISA KEV catalog — best-effort initial sync so the very first request
    # doesn't pay the fetch latency, and so a feed outage at boot is a
    # logged warning (kev.py's own lookups degrade gracefully after this)
    # rather than a startup crash. See app/services/risk/kev.py's module
    # docstring for what this does and does not cover.
    from app.services.risk import kev as _kev
    try:
        state = await _kev.sync()
        if state.fetch_ok:
            logger.info("cisa_kev_synced", entry_count=len(state.entries_by_cve), catalog_version=state.catalog_version)
        else:
            logger.warning("cisa_kev_sync_failed_at_startup", error=state.fetch_error)
    except Exception as exc:
        logger.warning("cisa_kev_sync_skipped", error=str(exc))
    ...
    from app.services.risk import kev as _kev_shutdown
    await _kev_shutdown.close()
    logger.info("ai_storage_service_shutdown")


app = FastAPI(
    title="ai-storage-service",
    description="Patchline X AI scanning + storage service — pluggable AI provider, MongoDB, Redis, Azure Blob.",
    version="1.0.0",
    lifespan=lifespan,
)
