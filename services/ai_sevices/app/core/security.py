...
class CurrentUser:
    def __init__(self, user_id: str, role: Optional[str] = None, org_id: Optional[str] = None, org_role: Optional[str] = None):
        self.id = user_id
        self.role = role
        # organization boundary — see authController.js/organizationService.js
        # in auth-service for how this claim is populated. Every Mongo query
        # this service issues on behalf of a real user token should be
        # scoped by org_id, not merely by user_id — see
        # pipeline_integration.user_can_manage_asset and the collections
        # migrated in risk/store.py for where that's actually enforced.
        self.org_id = org_id
...
async def require_auth(
    ...
    # Same enforcement as both Node services' middleware — a token with no
    ...
    if not org_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token missing organization claim — please log in again")
    user = CurrentUser(user_id=payload["sub"], role=payload.get("role"), org_id=org_id, org_role=payload.get("orgRole"))
...
async def require_auth_optional(
    ...
    x_system_org_id: Optional[str] = Header(default=None),
    ...
    if (
        ...
        # KNOWN GAP, not silently papered over: webhook/system-triggered
        # scans (main-service's BullMQ worker, see workers/scannerWorkers.js)
        # have no live JWT and therefore no org claim from the normal path.
        # x_system_org_id is OPTIONAL here specifically because the worker
        # doesn't send it yet — watchedRepoStore (main-service) has no
        # organizationId field to source it from. Until that's migrated too,
        # a system-triggered scan runs with org_id=None, and any code path
        # that requires an org_id (e.g. the org-scoped collections in
        # risk/store.py) will reject it rather than silently defaulting to
        # some org — see that module's own handling of org_id=None.
        request.state.user_id = x_system_user_id
        return CurrentUser(user_id=x_system_user_id, role="system", org_id=x_system_org_id, org_role="system")
