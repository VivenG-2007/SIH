...
function isWebhookConfigured() {
  return Boolean((env.github.webhookSecret && env.github.webhookUrl) || env.githubApp.webhookSecret);
}

// Proves a webhook POST actually came from GitHub — checked against the
// legacy per-repo hook secret (classic OAuth flow) OR the single App-level
// webhook secret (GitHub App flow, config/githubApp.js), since both kinds
// of connection can co-exist during the migration and both deliver to the
// same POST /api/github/webhook endpoint. Must be checked against the RAW
...
function verifyWebhookSignature(rawBody, signatureHeader) {
  ...
  const candidateSecrets = [env.github.webhookSecret, env.githubApp.webhookSecret].filter(Boolean);
...
async function exchangeCodeForToken(code) {
  const response = await fetchWithTimeout(TOKEN_URL, {
    method: 'POST',
    headers: { 'content-type': 'application/json', accept: 'application/json', 'user-agent': 'patchlinex' },
    body: JSON.stringify({
      client_id: env.github.clientId,
