...
function upstreamHeaders({ authHeader, requestId, userId, organizationId }) {
  const headers = {
  ...
  if (authHeader) {
    ...
    // organizationId comes from the watched_repositories row (P0#1 follow-
    // up) — may be null for a watch registered before that column existed;
    // ai-storage-service tolerates a missing org claim on this specific
    // system-token path rather than guessing one (see core/security.py's
    // own comment on this exact gap).
    if (organizationId) headers['x-system-org-id'] = organizationId;
  }
  return headers;
}
...
async function processScanJob(job) {
  const { scanId, userId, organizationId, repoOwner, repoName, branch, githubToken, authHeader, requestId, changedFiles, watchedRepositoryId, commitSha } = job.data;
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${env.aiStorageServiceUrl}/api/v1/scanner/scan`, {
      method: 'POST',
      headers: upstreamHeaders({ authHeader, requestId, userId, organizationId }),
...
async function processFixJob(job) {
  ...
  try {
    ...
    if (!fixResult.verified) {
      // Not a job failure — the fix genuinely didn't verify. Bounded retries
      // (see fixQueue.add attempts/backoff in the controller) exist for
      // transient failures, not for "the AI's patch didn't pass verification";
      // that outcome is final and goes back to a human for manual review.
      await scanStore.transitionFix(scanId, findingId, 'FIX_NEEDS_REVIEW', {
        verified: false,
