...
async function triggerScan(req, res, next) {
  try {
    ...
    const { accessToken } = await githubService.resolveToken(req.user.id);
...
async function getScanStatus(req, res, next) {
  try {
    ...
    return res.status(200).json({ scanId, ...scanRecord });
  } catch (err) {
    return next(err);
  }
}
...
async function approveAndFix(req, res, next) {
  try {
    ...
    const { accessToken } = await githubService.resolveToken(req.user.id);
    // accessToken is captured now and carried in the job payload rather than
    // re-resolved inside the worker — fine for a GitHub App installation
    // token (1hr TTL) given fixQueue's bounded retry window (3 attempts,
    // exponential backoff from 5s) is nowhere close to that.
