...
async def main():
    config = uvicorn.Config(app, host="127.0.0.1", port=8736, log_level="warning")
    server = uvicorn.Server(config)
    server_task = asyncio.create_task(server.serve())
    await asyncio.sleep(1.5)

    async with httpx.AsyncClient(base_url="http://127.0.0.1:8736", timeout=30) as client:
        baseline_latencies = []
        for _ in range(30):
    ...
    print("\n=== [PROCESS-POOL-OFFLOADED] /health latency, NO concurrent heavy load ===")
    print(f"  p50={pctl(baseline_latencies, 0.5):.1f}ms  p95={pctl(baseline_latencies, 0.95):.1f}ms")

    print("\n=== [PROCESS-POOL-OFFLOADED] /health latency, WITH 20 concurrent /heavy in flight ===")
    print(f"  p50={pctl(health_latencies_under_load, 0.5):.1f}ms  p95={pctl(health_latencies_under_load, 0.95):.1f}ms")

    degradation = pctl(health_latencies_under_load, 0.95) / pctl(baseline_latencies, 0.95)
    print(f"\np95 degradation factor: {degradation:.1f}x  (vs 85.6x no-offload, 88.5x thread-pool)")
